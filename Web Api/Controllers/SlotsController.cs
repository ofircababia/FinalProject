...
namespace Web_Api.Controllers
{
    ...
    public class SlotsController : ControllerBase
    {
        ...
        [HttpGet("date/{flightDate}")]
        public async Task<ActionResult<IEnumerable<Slot>>> GetSlotsByDate(DateTime flightDate)
        {
            var slots = await db.Slots
                                      .Where(s => s.FlightDate.HasValue && s.FlightDate.Value.Date == flightDate.Date)
                                      .ToListAsync();

            if (slots == null || slots.Count == 0)
            {
                return NotFound($"No slots found for the date: {flightDate.ToShortDateString()}");
            }
